...
class Analytics:
	...
	def run(self):
		...
		skip_total_row = 0
		if self.filters.tree_type in ["Customer Group", "Territory", "Item Group"]:
			skip_total_row = 1
	...
	def get_sales_transactions_based_on_sales_person(self):
		if self.filters["value_quantity"] == "Value":
			value_field = "st.allocated_amount"
		else:
			value_field = "si.total_qty"

		doc_type = self.filters.doc_type

		# Sales Team child table name is always "Sales Team"
		self.entries = frappe.db.sql(
			f"""
			SELECT
				st.sales_person AS entity,
				st.sales_person AS entity_name,
